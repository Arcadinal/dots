...
export WOFI_STYLE="$HOME/.config/wofi/style.css"
...
manage_wifi() {
    ...
    local chosen_network=$(echo -e "$formatted_list" | wofi --dmenu -selected-row 1 --prompt "Wi-Fi SSID: " --style "$WOFI_STYLE")
    local ssid_index=-1
    ...
    if [ -z "$chosen_network" ]; then
        ...
        action=$(echo -e "$action\n  Forget" | wofi --dmenu --prompt "Action: " --style "$WOFI_STYLE")
        case $action in
            "󰸋  Connect")
                ...
                if [[ $(echo "$saved_connections" | grep -Fx "$chosen_id") ]]; then
                    ...
                    local wifi_password=$(wofi --dmenu --prompt "Password: " --password --style "$WOFI_STYLE")
...
manage_ethernet() {
    ...
    local chosen_device=$(echo -e "$eth_list" | wofi --dmenu --prompt "Select Ethernet device: " --style "$WOFI_STYLE")
...
main_menu() {
    ...
    ##==> Выводим wofi меню
    #######################################################
    local chosen_option=$(echo -e "$wifi_toggle$manage_wifi_btn\n󱓥 Manage Ethernet" | wofi --dmenu --prompt " Network Management: " --style "$WOFI_STYLE")
